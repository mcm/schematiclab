...
const nbtTagTests: TagCase[] = [
  ...
  { name: "Int", tag: new nbt.Int(1), repr: "Int(1)", bytes: b("00000001") },
  {
    ...
    tag: new nbt.Long(1n),
    repr: "Long(1)",
    bytes: b("0000000000000001"),
  },
  {
    name: "Float",
    tag: new nbt.Float(1.0),
    repr: "Float(1)",
    bytes: b("3f800000"),
  },
  {
    name: "Double",
    tag: new nbt.Double(1.0),
    repr: "Double(1)",
    bytes: b("3ff0000000000000"),
  },
  {
    name: "String",
    tag: new nbt.String("foo"),
    repr: 'String("foo")',
    bytes: concat(b("0003"), "foo"),
  },
  {
    name: "ByteArray",
  ...
  },
  {
    ...
    bytes: b(
      "00000003" + "0000000000000001" + "0000000000000002" + "0000000000000003",
    ),
  },
  {
    name: "empty List",
  ...
  },
  {
    name: "Named",
    tag: new nbt.Named({
      "": new nbt.Compound({ foo: new nbt.String("bar") }),
    }),
...
describe("NbtTag round-trip", () => {
  it.each(nbtTagTests)("$name -> fromBytes", ({ tag, bytes }) => {
    const cls = tag.constructor as unknown as {
      fromBytes(b: Uint8Array): nbt.NbtTag;
    };
...
describe("List", () => {
  it("from_bytes with invalid item type throws", () => {
    ...
    expect(() =>
      nbt.List.fromBytes(b("a30000000300000001000000020000000300")),
    ).toThrow();
  });
});
...
describe("Compound", () => {
  it("from_bytes with invalid item type throws", () => {
    expect(() =>
      nbt.Compound.fromBytes(concat(b("a3"), b("0003"), "foo")),
    ).toThrow();
  });
});
...
describe("loadNbtFromBytes", () => {
  it("parses a Named root", () => {
    ...
    expect(
      named.equals(
        new nbt.Named({ "": new nbt.Compound({ foo: new nbt.String("bar") }) }),
      ),
    ).toBe(true);
  });
});
...
describe("tag type registry", () => {
  ...
  it("from_buff on base NbtTag throws (no implementation)", () => {
    expect(() =>
      nbt.NbtTag.fromReader(new nbt.BinaryReader(new Uint8Array(0))),
    ).toThrow();
  });
});
